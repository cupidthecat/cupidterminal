...
static void query_with_capture(const char *seq, ResponseCapture *cap) {
    cap->len = 0;
    twrite((const uint8_t *)seq, strlen(seq), &term, capture_response, cap);
}
...
int main(void) {
    ...
    cap.len = 0;
    twrite((const uint8_t *)utf8_gear, strlen(utf8_gear), &term, capture_response, &cap);
    ...
    test_reset_terminal(2, 8);
    cap.len = 0;
    twrite((const uint8_t *)utf8_block, strlen(utf8_block), &term, capture_response, &cap);
    ...
    test_reset_terminal(2, 8);
    cap.len = 0;
    twrite((const uint8_t *)utf8_elem, strlen(utf8_elem), &term, capture_response, &cap);
    ...
    test_reset_terminal(2, 5);
    test_assert_mode("cursor_visible", term.cursor_visible, 1);
    test_feed_string("\x1b[?25l");
    test_assert_mode("cursor_visible", term.cursor_visible, 0);
    ...
    test_assert_mode("autowrap_mode", term.autowrap_mode, 0);
    test_feed_string("ABCDE");
    ...
    test_assert_mode("autowrap_mode", term.autowrap_mode, 1);
    test_feed_string("Y");
    ...
    test_assert_mode("origin_mode", term.origin_mode, 1);
    ...
    test_assert_mode("insert_mode", term.insert_mode, 0);
