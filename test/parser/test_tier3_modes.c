...
int main(void) {
    ...
    test_assert_mode("bracketed_paste_mode", term.bracketed_paste_mode, 1);
    payload_len = tpastefmt((const uint8_t *)"abc", 3, term.bracketed_paste_mode, payload, sizeof(payload));
    assert_payload(payload, payload_len, "\x1b[200~abc\x1b[201~", 15, "bracketed paste payload mismatch");

    test_feed_string("\x1b[?2004l");
    test_assert_mode("bracketed_paste_mode", term.bracketed_paste_mode, 0);
    payload_len = tpastefmt((const uint8_t *)"abc", 3, term.bracketed_paste_mode, payload, sizeof(payload));
    assert_payload(payload, payload_len, "abc", 3, "plain paste payload mismatch");

    test_feed_string("\x1b[?1000h\x1b[?1006h");
    test_assert_mode("mouse_reporting_basic", term.mouse_reporting_basic, 1);
    test_assert_mode("mouse_sgr_mode", term.mouse_sgr_mode, 1);
    test_feed_string("\x1b[?1000l\x1b[?1006l");
    test_assert_mode("mouse_reporting_basic", term.mouse_reporting_basic, 0);
    test_assert_mode("mouse_sgr_mode", term.mouse_sgr_mode, 0);

    test_feed_string("\x1b[?1h");
    test_assert_mode("application_cursor_keys", term.application_cursor_keys, 1);
    test_feed_string("\x1b[?1l");
    test_assert_mode("application_cursor_keys", term.application_cursor_keys, 0);

    test_feed_string("\x1b]0;title-one\x07");
    test_assert_true(strcmp(term.window_title, "title-one") == 0, "OSC 0 title mismatch");
    test_assert_mode("title_dirty", term.title_dirty, 1);

    term.title_dirty = 0;
    test_feed_string("\x1b]2;title-two\x1b\\");
    test_assert_true(strcmp(term.window_title, "title-two") == 0, "OSC 2 title mismatch");
    test_assert_mode("title_dirty", term.title_dirty, 1);

    term.title_dirty = 0;
    test_feed_bytes(osc_part1, sizeof(osc_part1));
    test_assert_mode("title_dirty", term.title_dirty, 0);
    test_feed_bytes(osc_part2, sizeof(osc_part2));
    test_assert_true(strcmp(term.window_title, "split") == 0, "split OSC title mismatch");
    test_assert_mode("title_dirty", term.title_dirty, 1);

    term.osc52_pending = 0;
    test_feed_string("\x1b]52;c;aGVsbG8=\x07");
    test_assert_mode("osc52_pending", term.osc52_pending, 1);
    test_assert_true(term.osc52_len == 5, "OSC 52 decoded length mismatch");
    test_assert_true(memcmp(term.osc52_buf, "hello", 5) == 0, "OSC 52 decoded payload mismatch");

    term.osc52_pending = 0;
    test_feed_bytes(osc52_part1, sizeof(osc52_part1));
    test_assert_mode("osc52_pending", term.osc52_pending, 0);
    test_feed_bytes(osc52_part2, sizeof(osc52_part2));
    test_assert_mode("osc52_pending", term.osc52_pending, 1);
    test_assert_true(term.osc52_len == 5, "split OSC 52 decoded length mismatch");
    test_assert_true(memcmp(term.osc52_buf, "world", 5) == 0, "split OSC 52 payload mismatch");
